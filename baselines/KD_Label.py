...
def para(args): 
    if args.dataset == 'yahooR3': 
        ...
        args.teacher_model_args = {'emb_dim': 10, 'learning_rate': 0.1, 'weight_decay': 0}
        ...
        args.teacher_model_args = {'emb_dim': 10, 'learning_rate': 0.1, 'weight_decay': 0.01}
    else: 
        print('invalid arguments')
        os._exit()
                

def train_and_eval(train_data, unif_data, val_data, test_data, device = 'cuda', model_class = MF, 
        base_model_args: dict = {'emb_dim': 10, 'learning_rate': 0.001, 'weight_decay': 0.0}, 
        teacher_model_args: dict = {'emb_dim': 10, 'learning_rate': 0.1, 'weight_decay': 0.0}, 
    ...
    teacher_model = MF(n_user, n_item, dim=teacher_model_args['emb_dim'], dropout=0).to(device)
    teacher_optimizer = torch.optim.SGD(teacher_model.parameters(), lr=teacher_model_args['learning_rate'], weight_decay=0)
    
    # loss_criterion
    criterion = nn.MSELoss(reduction='sum')

    # begin base model training
    stopping_args = Stop_args(patience=training_args['patience'], max_epochs=training_args['epochs']//1)
    ...
    for epo in range(early_stopping.max_epochs):
        ...
        for u_batch_idx, users in enumerate(unif_loader.User_loader): 
            for i_batch_idx, items in enumerate(unif_loader.Item_loader): 
                ...
                loss = criterion(y_hat, y_train) + teacher_model_args['weight_decay'] * teacher_model.l2_norm(users_train, items_train)
...
if __name__ == "__main__": 
    ...
    train_and_eval(train, unif_train, validation, test, device, base_model_args = args.base_model_args, teacher_model_args = args.teacher_model_args, training_args = args.training_args)
